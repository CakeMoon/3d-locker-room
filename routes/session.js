...
router.post(
  ...
  try {
    ...
    let user = await Users.findOneName(req.body.username);
    ...
    if (user.password != req.body.password) {
      res.status(403).json({ error: `Password does not match` }).end();
      return;
    }
    
    // authenticate and sign the user in
    req.session.uid = user.id;
    res.status(200).json({data: user, message: "You are signed in."}).end();
